...
class WellDatasetForSequence(Dataset):
    """
    Auto-discovers HDF5 shards and yields a single contiguous clip from full trajectories.

    This is useful for models that consume one input sequence directly rather than a
    JEPA-style (context, target) pair.

    Returns:
      dict with 'sequence': (C,T,H,W) and 'physical_params' (tensor).
    """
    ...
    def __init__(
        ...
        stride: int = None,
        subset_config_path: Optional[str | Path] = None,
        noise_std: float = 0.0,
        max_open_files: int = 6,
        rdcc_nbytes: int = 512 * 1024**2,
        rdcc_nslots: int = 1_000_003,
        ...
        if len(self.index) == 0:
            raise ValueError("No valid windows found. "
                             "Check num_frames and that trajectories have at least num_frames frames.")
    ...
    def _build_index(self) -> Tuple[List[tuple[int, int, int]], Dict[str, List[np.ndarray]]]:
        ...
        for path in paths:
            with h5py.File(path, 'r') as f:
                example_scalar_field = f['t0_fields'][list(f['t0_fields'].keys())[0]]
                T = int(example_scalar_field.shape[1])
                max_t0 = T - F
                if max_t0 < 0:
                    continue
                stride = self.stride if self.stride is not None else F
                for obj_id in range(example_scalar_field.shape[0]):
                    for t0 in range(0, max_t0 + 1, stride):
                        idx.append((path.name, obj_id, t0))
                physical_params_idx[path.name] = [f['scalars'][key][()] for key in f['scalars'].keys() if key != "L"]
    ...
    def __getitem__(self, i):
        ...
        seq = np.empty((F, H, W, C), dtype=self._dtype, order="C")
        ...
        sel_f_prefix = (local_obj_idx, slice(t0, t0 + F), h_slice, w_slice)
        tmp_cache = state.setdefault("sequence_buf_cache", {})
        ...
        for path, dsize, comp_shape in zip(self._field_paths, self._d_sizes, self._comp_shapes):
            ...
            sel = sel_f_prefix + (slice(None),) * len(comp_shape)
            ds.read_direct(buf, source_sel=sel)

            view = buf.reshape(F, H, W, dsize)
            c1 = c0 + dsize
            seq[..., c0:c1] = view
            c0 = c1

        seq_t = torch.from_numpy(seq).permute(3, 0, 1, 2).contiguous()

        if self.resolution is not None and tuple(seq_t.shape[-2:]) != tuple(self.resolution):
            seq_t = torch.nn.functional.interpolate(seq_t, size=self.resolution, mode='bilinear', align_corners=False)

        if self.noise_std > 0:
            seq_t = seq_t + torch.randn_like(seq_t) * self.noise_std

        return {"sequence": seq_t, "physical_params": torch.tensor(self.physical_params_idx[file_id])}
...
def get_test_dataloader_from_cfg(cfg, stage="train", rank=None, world_size=None):
...
def get_train_sequence_dataloader_from_cfg(cfg, stage="train", rank=None, world_size=None, num_frames=None):
    return get_train_sequence_dataloader(
        cfg.dataset.name,
        num_frames if num_frames is not None else cfg.dataset.num_frames,
        cfg.dataset.get("num_examples", None),
        cfg[stage].batch_size,
        rank=rank if rank is not None else 0,
        world_size=world_size if world_size is not None else 1,
...
def get_test_sequence_dataloader_from_cfg(cfg, stage="train", rank=None, world_size=None, num_frames=None):
    return get_test_sequence_dataloader(
        cfg.dataset.name,
        num_frames if num_frames is not None else cfg.dataset.num_frames,
        cfg.dataset.get("num_examples", None),
        cfg[stage].batch_size,
...
def get_val_sequence_dataloader_from_cfg(cfg, stage="train", rank=None, world_size=None, num_frames=None):
    return get_val_sequence_dataloader(
        cfg.dataset.name,
        num_frames if num_frames is not None else cfg.dataset.num_frames,
        cfg.dataset.get("num_examples", None),
        cfg[stage].batch_size,
...
def get_train_sequence_dataloader(
        dataset_name,
        num_frames,
        num_examples,
        batch_size,
    ...
    if rank == 0:
        print(f"total number of sequence clips in train dataset: {len(dataset)}")
    if world_size == 1:
        sampler = None
    else:
        sampler = DistributedSampler(
...
def get_val_sequence_dataloader(
        dataset_name,
        num_frames,
        num_examples,
        batch_size,
